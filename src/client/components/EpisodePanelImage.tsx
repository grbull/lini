...
export function EpisodePanelImage({ alt, imageURL }: Props): ReactElement {
  ...
  const [imageHeight, setImageHeight] = useState((295 / 210) * 100);
  ...
  return (
    <div
      className={cn('w-full', 'h-0', 'relative', 'rounded-sm')}
      style={{ paddingTop: `${imageHeight}%` }}
    >
      <img
        alt={alt}
        className={cn('w-full', 'absolute', 'top-0', 'left-0', 'rounded-sm')}
        src={image}
      />
    </div>
  );
